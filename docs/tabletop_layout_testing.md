...
rendered board, point targets, rail separation, horizontal overflow, action
button overlap, and the scroll fallback below the supported minimum tier.
...
- shared local play from both ends of the phone, including rail rotation,
  turn-state changes, rail-owned resignation, confirmation-sheet direction,
  and both halves of the result overlay.
